...
function Tabs() {
  ...
  const handleClick = (category,index) => {
    ...
    const categoryData = cards[0]?.table_menu_list?.find(categoryData => categoryData.menu_category === category);
      ...
      <div className="flex pt-4 relative items-center ">
        ...
        >
          {cards[0]?.table_menu_list?.map((dishCategories, index) => {
            ...
            } cursor-pointer hover:scale-105 ease-in-out duration-300 w-36 mx-2 sm:mx-0 sm:w-1/4`;
            return (
              <button
                className={buttonClasses}
                onClick={() => handleClick(dishCategories?.menu_category,index)}
                key={index}
              >
